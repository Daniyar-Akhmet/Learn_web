...
def process_login():
    ...
    if form.validate_on_submit():  # провкрка на ошибки, могут возникнуть при вводе данных пользователем в форму
        user = User.query.filter(User.username == form.username.data).first()
        if user and user.check_password(form.password.data):
            login_user(user, remember=form.remember_me.data)
            flash('Вы успешно зашли на сайт') # позволяет передавать сообщения между route-ами
            return redirect(url_for('news.index'))  #делает перенаправление пользователя на другую страницу
